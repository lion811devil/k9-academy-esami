...
begin
 ...
 if ex.status='assigned' then
  ...
  set status='in_progress',started_at=now(),ends_at=now()+make_interval(mins=>duration_minutes)
  ...
  -- Composizione nuova per ogni assegnazione:
  -- 1) randomizza le domande all'interno di ogni categoria;
  -- 2) distribuisce il prelievo fra le categorie disponibili;
  -- 3) rimescola l'ordine finale della prova.
  ...
  'questions',(
    select jsonb_agg(
      jsonb_build_object(
        ...
        'correct_option',case when eq.selected_option is not null then qb.correct_option else null end,
        'explanation',case when eq.selected_option is not null then qb.explanation else null end
