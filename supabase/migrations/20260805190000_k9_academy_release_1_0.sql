-- K9 ACADEMY ESAMI RELEASE 1.0 - SCHEMA COMPLETO
...
create table if not exists public.exam_questions(
 ...
 question_id bigint references public.question_bank(id),selected_option smallint check(selected_option between 0 and 3),is_correct boolean,answered_at timestamptz,
...
create table if not exists public.practical_evaluations(
 ...
 professional_score integer default 0 check(professional_score between 0 and 20),notes text,completed_at timestamptz default now());
...
begin
 ...
 if ex.status='assigned' then
  update public.exam_assignments set status='in_progress',started_at=now(),ends_at=now()+make_interval(mins=>duration_minutes) where id=ex.id returning * into ex;
  ...
  insert into public.exam_questions(assignment_id,position,question_id)
  select ex.id,row_number() over(),id
  from (
 ...
 elsif ex.status<>'in_progress' then raise exception 'Esame non avviabile'; end if;
 if now()>=ex.ends_at then perform public.finalize_exam(ex.id,true); raise exception 'Tempo scaduto'; end if;
 return jsonb_build_object('assignment_id',ex.id,'discipline',ex.discipline,'started_at',ex.started_at,'ends_at',ex.ends_at,'question_count',ex.question_count,'duration_minutes',ex.duration_minutes,'status',ex.status,
 'questions',(select jsonb_agg(jsonb_build_object('position',eq.position,'code',qb.code,'category',qb.category,'difficulty',qb.difficulty,'question_text',qb.question_text,'options',jsonb_build_array(qb.option_a,qb.option_b,qb.option_c,qb.option_d),'selected_option',eq.selected_option,'is_correct',eq.is_correct,'correct_option',case when eq.selected_option is not null then qb.correct_option else null end,'explanation',case when eq.selected_option is not null then qb.explanation else null end) order by eq.position) from public.exam_questions eq join public.question_bank qb on qb.id=eq.question_id where eq.assignment_id=ex.id));
end$$;

create or replace function public.answer_exam_question(p_assignment_id uuid,p_position integer,p_selected_option integer)
returns jsonb language plpgsql security definer set search_path=public as $$declare ex public.exam_assignments%rowtype;eq public.exam_questions%rowtype;q public.question_bank%rowtype;ok boolean;
begin
 ...
 select * into q from public.question_bank where id=eq.question_id;ok:=(p_selected_option=q.correct_option);
 update public.exam_questions set selected_option=p_selected_option,is_correct=ok,answered_at=now() where assignment_id=p_assignment_id and position=p_position;
 update public.exam_assignments set answered_questions=(select count(*) from public.exam_questions where assignment_id=p_assignment_id and selected_option is not null),correct_answers=(select count(*) from public.exam_questions where assignment_id=p_assignment_id and is_correct=true) where id=p_assignment_id;
 return jsonb_build_object('is_correct',ok,'correct_option',q.correct_option,'explanation',q.explanation);end$$;
...
select e.*,s.full_name student_name,s.email student_email,a.full_name assigned_by_name,ev.full_name evaluator_name,ev.email evaluator_email,p.practical_score,p.professional_score,p.notes practical_notes
from public.exam_assignments e join public.profiles s on s.id=e.student_id join public.profiles a on a.id=e.assigned_by left join public.profiles ev on ev.id=e.evaluator_id left join public.practical_evaluations p on p.assignment_id=e.id;
