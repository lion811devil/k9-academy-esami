...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (profileError || !["super_admin", "vice_admin"].includes(callerProfile?.role || "")) {
      return json({ error: "Funzione riservata all'amministrazione." }, 403);
    }
    ...
    if (action === "create_staff_user") {
      ...
      const allowedForVice = ["student", "teacher", "examiner", "vice_admin"];
      const allowedForSuper = [...allowedForVice, "super_admin"];
      const allowed = isSuper ? allowedForSuper : allowedForVice;
      if (!fullName) throw new Error("Inserisci nome e cognome.");
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new Error("Email non valida.");
      if (password.length < 8) throw new Error("La password deve contenere almeno 8 caratteri.");
      if (!allowed.includes(requestedRole)) throw new Error(isSuper ? "Ruolo non valido." : "Il Vice Amministratore non può creare Super Amministratori.");
