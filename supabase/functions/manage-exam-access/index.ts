// K9 Academy Esami — Release 1.77 — percorso Corsista > Corso > Esame > Documenti
  ...
  try {
    ...
    if (action === "create_course_with_students") {
      ...
      const { data: session, error: sessionError } = await admin.from("exam_sessions").insert({
        title, common_username: username, discipline, question_count: questionCount,
        duration_minutes: durationMinutes, pass_percentage: passPercentage, notes: body.notes || null, created_by: authData.user.id,
      }).select("*").single();
      if (sessionError || !session) throw sessionError || new Error("Creazione corso non riuscita.");
      ...
      try {
        ...
        for (const studentId of studentIds) {
          ...
          if (assignmentError || !assignment) throw assignmentError || new Error("Assegnazione prova non riuscita.");
